...
public:
    ManagerProxy(sdbus::IConnection &connection, const sdbus::ServiceName &destination, sdbus::ObjectPath path)
    ...
    void handleExistingObjects() {
        for (auto objectsInterfacesAndProperties = GetManagedObjects(); const auto &[object, interfacesAndProperties]:
             objectsInterfacesAndProperties) {
...
private:
    void onInterfacesAdded(const sdbus::ObjectPath &objectPath
        ...
        auto planetInterface = interfacesAndProperties.find(sdbus::InterfaceName{
            org::sdbuscpp::ExampleManager::Planet1_proxy::INTERFACE_NAME
        });
...
int main() {
    ...
    const auto managerProxy = std::make_unique<
        ManagerProxy>(*connection, std::move(destination), std::move(objectPath));
    try {
